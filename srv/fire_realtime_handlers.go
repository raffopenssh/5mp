...
func (s *Server) HandleAPIFireRealtime(w http.ResponseWriter, r *http.Request) {
	...
	// Note: this handler ends here. A ~120-line pre-v5 implementation used to
	// sit below an unconditional `return` (go vet: unreachable code) -- it
	// clustered raw fire_detections at request time and minted sequential
	// _grp_N ids that the features API cannot resolve. Deleted 2026-08-10;
	// feature_geometries (the v5 pipeline's output) is the only source. Git
	// history has it if you ever want to read it.
}
