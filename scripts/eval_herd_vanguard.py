...
for g in raw:
    ...
    if g.get("group_type") == "transhumance" and float(g.get("distance_km") or 0) >= 150:
        T.append(dict(t=t, s=season_of(g["start_date"]), tier=g.get("evidence_tier"), van=bool(g.get("vanguard")), lead=g.get("lead_start"), bits=g.get("evidence_bits"), fires=g["fire_count"], days=g["days"]))
print("long fronts", len(T), Counter(x["s"] for x in T))
...
season = np.array([x["s"] for x in T]); van = np.array([x["van"] for x in T]); tier = np.array([x["tier"] for x in T])
...
def run(name, fit_sel, weights=None, lab=None, capture_min=0.5):
    ...
    fit = (season != HOLD) & fit_sel; hold = season == HOLD
    if lab is None: lab = kmeans(fit)
    n_fit = int(fit.sum()); band_all = np.zeros((G.h, G.w), bool); rows = []
    for b in range(K):
        fi = np.flatnonzero(fit & (lab == b)); hi = np.flatnonzero(hold & (lab == b)); hv = np.flatnonzero(hold & (lab == b) & van)
        ...
        rows.append(dict(b=b, fit=len(fi), hold=len(hi), hold_van=len(hv), km2=best["km2"], q=best["q"], cap=best["hit"], nul=best["nul"], skill=best["skill"],
                         cap_van=capture(hv, band), nul_van=capture(hv, top_n(alld_s, int(band.sum()))), cap_nonvan=capture(np.flatnonzero(hold & (lab == b) & ~van), band)))
    hi_all = np.flatnonzero(hold); hv_all = np.flatnonzero(hold & van); n_ = int(band_all.sum()); null_all = top_n(alld_s, n_)
    net = dict(name=name, fit=n_fit, bundles=len(rows), km2=n_ * G.cell_km2(), share=n_ / G.mask.sum(), cap=capture(hi_all, band_all), nul=capture(hi_all, null_all),
               cap_van=capture(hv_all, band_all), nul_van=capture(hv_all, null_all), cap_nonvan=capture(np.flatnonzero(hold & ~van), band_all),
    ...
    print(f"   hold-out ALL {len(hi_all)}: capture {net['cap']:.2f} (all-fire null {net['nul']:.2f}) | VANGUARD {len(hv_all)}: {net['cap_van']:.2f} (null {net['nul_van']:.2f}) | non-vanguard {net['cap_nonvan']:.2f}")
    ...
    for r in rows: print(f"   b{r['b']:>2} fit {r['fit']:>4} hold {r['hold']:>4} (van {r['hold_van']:>3}) q{r['q']} {r['km2']:>7,.0f} km2 cap {r['cap']:.2f} null {r['nul']:.2f} skill {r['skill']:+.2f} | van cap {r['cap_van']:.2f} (null {r['nul_van']:.2f}) non-van {r['cap_nonvan']:.2f}")
    return net, rows, lab
res = {}
res["A_current"] = run("A current model (all long fronts)", np.ones(len(T), bool))
labA = res["A_current"][2]
res["B_supported"] = run("B supported tier only (fire.md 'downstream to revisit')", tier == "supported", lab=labA)
res["C_supp_weak"] = run("C supported+weak", np.isin(tier, ["supported", "weak"]), lab=labA)
...
res["D_leadw"] = run("D lead-weighted UD (ahead-of-front fronts up to 4x)", np.ones(len(T), bool), weights=w, lab=labA)
res["E_prefront"] = run("E fit only fronts starting ahead of the front (lead>=0) + 2023 (no front)", (lead >= 0) | np.isnan(lead), lab=labA)
# tier composition of held-out vanguard fronts
print("\nheld-out 2025 vanguard fronts by tier:", Counter(tier[(season == HOLD) & van]))
print("held-out 2025 all by tier:", Counter(tier[(season == HOLD)]))
...
print("\n== direction: bundle heading from ALL fronts (day order) vs its vanguard chains (order measured trustworthy)")
agree_tot = n_tot = 0
for b in range(K):
    ai = np.flatnonzero(lab_ := (labA == b)); vi = np.flatnonzero((labA == b) & van)
    if len(ai) < 0.03 * (season != HOLD).sum() or len(vi) < 5: continue
    ...
    print(f"   b{b:>2}: all {len(ai):>4} heading {ha:5.0f}° (R {ra:.2f}, {agree_all:.0%} within 90°) | vanguard {len(vi):>3} heading {hv_:5.0f}° (R {rv:.2f}) Δ {diff:3.0f}°, {agree:.0%} of vanguard within 90° of bundle")
print(f"   overall: {agree_tot/n_tot:.0%} of vanguard chains head within 90° of their bundle's mean heading (50% = no agreement)")
