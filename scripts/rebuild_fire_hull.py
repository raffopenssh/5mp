#!/usr/bin/env python3
"""
Fire Convex Hull Progression Trajectory Builder

Instead of centroid paths, tracks the CONVEX HULL edge progression.
For each time window, computes the convex hull and tracks its leading edge.

Output: data/fire_groups_hull/{park_id}.json
"""
...
OUTPUT_DIR = BASE_DIR / "data" / "fire_groups_hull"
...
def get_hull_leading_edge(fires, overall_bearing):
    """
    Compute convex hull of fires and find the point on the hull
    furthest in the direction of movement.
    """
    if not fires:
        return None
    if len(fires) == 1:
    ...
    # Get unique points
    points = list(set((f['longitude'], f['latitude']) for f in fires))
    
    if len(points) < 3:
        # Not enough for hull, use furthest point
        ...
        best = max(points, key=lambda p: (p[0]-cx)*dx + (p[1]-cy)*dy)
        return best
    
    # Compute convex hull
    hull = convex_hull(points)
    
    # Find hull point furthest in direction of movement
    ...
    best = max(hull, key=lambda p: (p[0]-cx)*dx + (p[1]-cy)*dy)
    return best

def build_hull_trajectory(fires):
    """Build trajectory tracking the convex hull leading edge."""
    if not fires:
        return []
    ...
    # Build trajectory using hull leading edge
    trajectory = []
    for key in sorted_windows:
        wf = fires_by_window[key]
        edge = get_hull_leading_edge(wf, overall_bearing)
        date, window = key
        mid_hour = window * WINDOW_HOURS + WINDOW_HOURS // 2
        trajectory.append([edge[0], edge[1], date, f"{mid_hour:02d}00"])
...
def analyze_group(cluster, park_id, park_geometry):
    ...
    trajectory = build_hull_trajectory(sorted_fires)
...
def main():
    ...
    log("Fire Convex Hull Trajectory Builder")
    log("Loading parks...")
    parks = load_parks()
    log(f"Loaded {len(parks)} parks")
    log(f"Params: {CLUSTER_KM}km cluster, {BASE_LINK_KM}+{LINK_KM_PER_DAY}km/day link, {MAX_GAP_DAYS}d gap")
