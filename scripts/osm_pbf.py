...
# Rest of the world: iso3 -> {region, url}, generated by
# scripts/build_geofabrik_countries.py from the Geofabrik index + Natural
# Earth. The African dict above stays authoritative where it overlaps (it
# knows the local staging dirs and groupings like senegal-and-gambia).
# User-drawn AOIs are not limited to the park continent (Serra Bonita/BRA,
# 2026-08-19); these PBFs always download to /tmp and are deleted by the
# caller after the area is processed -- only data/osm_geofabrik/ is a cache.
GLOBAL_INDEX = Path(__file__).resolve().parent.parent / "data" / "geofabrik_countries.json"
_global = None


def global_region(iso):
    """{'region':…, 'url':…} for an ISO3 outside GEOFABRIK, or None."""
    global _global
    if _global is None:
        _global = json.loads(GLOBAL_INDEX.read_text()) if GLOBAL_INDEX.exists() else {}
    return _global.get(iso)
...
def ensure_pbf(iso, dest_dir="/tmp"):
    ...
    African parks countries come from GEOFABRIK (and may be staged locally in
    LOCAL_PBF_DIRS); any other country resolves through the generated global
    index. Global downloads land in /tmp and report is_temporary=True, so the
    caller deletes them after the area is processed -- a one-off AOI must not
    leave a 1-4 GB PBF behind.
    """
