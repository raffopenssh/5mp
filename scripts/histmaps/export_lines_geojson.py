...
with open(lines_out, "w") as f:
    ...
    first = True
    for (lid, kind, style, name, names_all, n_segs, n_sheets, sheets,
         year_min, year_max, length_km, pts) in c.execute(
            "SELECT id,kind,style,name,names_all,n_segs,n_sheets,sheets,"
            " year_min,year_max,length_km,pts FROM lines_stitched"):
        feat = {"type": "Feature",
                "geometry": {"type": "LineString",
                             "coordinates": json.loads(pts)},
                "properties": {"id": lid, "kind": kind, "style": style,
                               "name": name, "names_all": names_all,
...
with open(symbols_out, "w") as f:
    f.write('{"type":"FeatureCollection","features":[\n')
    first = True
    for sid, sheet, descr, cat, lon, lat in c.execute(
            "SELECT id, sheet, descr, category, lon, lat FROM symbols"):
        feat = {"type": "Feature",
