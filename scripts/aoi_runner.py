...
# Country PBFs downloaded this run, path -> aoi_id. A PBF is already
# zlib-compressed protobuf (recompressing saves ~1%), and keeping raw country
# extracts around forever does not scale (Brazil alone is 2 GB, and they go
# stale daily) -- so the policy is: keep while the AOI still needs it, sweep
# when it does not. Deleting per-unit made the osm and hydro units of one AOI
# download the same 2 GB Brazil PBF twice five minutes apart (2026-08-19),
# and sweeping unconditionally at exit has the same failure across slices: a
# partial slice would re-download on resume tomorrow. The sweep therefore
# checks the queue: while the AOI still has unfinished PBF-consuming datasets
# (osm, hydro), the file stays in /tmp; once they are done it is deleted.
# The deliberately-staged local PBFs (data/osm_geofabrik/: CAF and SSD) are
# never listed here (ensure_pbf returns temporary=False for them).
_run_pbfs = {}

PBF_DATASETS = ("osm", "hydro")


def keep_for_run(path, aoi_id=None):
    _run_pbfs[path] = aoi_id


def sweep_run_pbfs(conn):
    for p, aid in list(_run_pbfs.items()):
        if aid:
            left = conn.execute(
                f"""SELECT COUNT(*) FROM aoi_datasets
                    WHERE aoi_id=? AND enabled=1 AND state!='done'
                      AND dataset IN ({','.join('?' * len(PBF_DATASETS))})""",
                (aid, *PBF_DATASETS)).fetchone()[0]
            if left:
                continue   # a later slice still needs it; leave it in /tmp
        try:
            os.remove(p)
        except OSError:
            pass
    ...
    while cur["i"] < total and time.time() < deadline:
        ...
        try:
            ...
            try:
                ...
                if temporary:
                    # kept while this AOI still has PBF-consuming datasets
                    # pending (hydro reuses it); swept by sweep_run_pbfs().
                    keep_for_run(pbf, aoi["id"])
        except Exception as ex:
    ...
    while cur["i"] < total and time.time() < deadline:
        ...
        try:
            ...
            n_r, n_l = osm_hydro.ingest_country(
                ...
                keep_pbf=lambda p: keep_for_run(p, aid))
            cur["rivers"] += n_r
            cur["lakes"] += n_l
        except Exception as ex:
            cur.setdefault("failed", []).append([iso, str(ex)[:80]])
...
def aoi_countries(conn, aoi):
    """ISO3 codes the AOI spans, anywhere on Earth.

    Natural Earth 50m polygons (data/world_countries.geojson, generated by
    scripts/build_geofabrik_countries.py) answer "which countries does this
    polygon touch". Only countries Geofabrik can serve as one extract are
    returned (GEOFABRIK for Africa, the generated global index elsewhere);
    an AOI in a country we cannot fetch gets 0 units for the osm/hydro
    datasets rather than a crash. Used to be derived from the parks the AOI
    intersects, which meant any AOI outside the park continent silently got
    no OSM at all (Serra Bonita/BRA, 2026-08-19).
    """
    import osm_pbf
    from shapely.geometry import shape
    geom = aoi_lib.aoi_geom(aoi)
    isos = set()
    with open(BASE_DIR / "data" / "world_countries.geojson") as f:
        for c in json.load(f)["features"]:
            try:
                if shape(c["geometry"]).intersects(geom):
                    isos.add(c["properties"]["iso3"])
            except Exception:
                continue
    return sorted(i for i in isos
                  if i in osm_pbf.GEOFABRIK or osm_pbf.global_region(i))
