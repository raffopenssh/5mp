#!/usr/bin/env python3
"""
Fire Trajectory Builder v4 - Clean trajectories with zigzag removal

Features:
- Tighter clustering (3km, 2-day gap)
- Direction consistency checking (60° max turn)
- Fire front tracking (leading edge, not centroid)
- Zigzag removal: trajectories with >30% zigzag segments converted to point clusters

Output: data/fire_groups_v4/{park_id}.json
"""
...
OUTPUT_DIR = BASE_DIR / "data" / "fire_groups_v4"
...
MAX_GAP_DAYS = 2
MIN_DATE = '2020-01-01'
MAX_DIRECTION_CHANGE = 60  # Max degrees for linking

# Trajectory quality thresholds
ZIGZAG_THRESHOLD = 90  # Degrees - anything > this is a zigzag
MAX_ZIGZAG_RATIO = 0.3  # Max 30% zigzag segments allowed
MIN_TRAJECTORY_POINTS = 2  # Need at least 2 points for a line
...
def cluster_fires_windowed(fires):
    """Cluster fires with direction consistency checking."""
    if not fires:
        return []
...
def get_fire_front(fires, overall_bearing):
    ...
    if not fires:
        return None
    if len(fires) == 1:
        return (fires[0]['longitude'], fires[0]['latitude'])
    ...
    best = max(fires, key=lambda f: (f['longitude']-cx)*dx + (f['latitude']-cy)*dy)
    return (best['longitude'], best['latitude'])

def build_trajectory(fires):
    """Build trajectory using fire front tracking."""
    if not fires:
        return []
...
def is_trajectory_clean(trajectory):
    """
    Check if trajectory has consistent direction.
    Returns (is_clean, zigzag_ratio, avg_change)
    """
    if len(trajectory) < MIN_TRAJECTORY_POINTS:
        return True, 0, 0  # Single points are always "clean"
...
def clean_trajectory(trajectory):
    """
    Remove zigzag sections from trajectory, keeping only consistent segments.
    If too many zigzags, return just the centroid as a single point.
    """
    if len(trajectory) < 3:
        return trajectory
    
    # Find longest consistent segment
    bearings = []
    for i in range(1, len(trajectory)):
        b = bearing(trajectory[i-1][0], trajectory[i-1][1],
                   trajectory[i][0], trajectory[i][1])
    ...
    # Find segments with consistent direction
    segments = []
    current_segment = [0]  # Start indices
    ...
    if not segments:
        ...
        return [[sum(lons)/len(lons), sum(lats)/len(lats), 
                 trajectory[0][2], trajectory[0][3]]]
    
    longest = max(segments, key=len)
    
    # Return trajectory for longest consistent segment
    # Include the point before and after the segment indices
    start_idx = longest[0]
    end_idx = longest[-1] + 1  # +1 because bearings are between points
...
def analyze_group(cluster, park_id, park_geometry):
    ...
    # Check if clean and clean if needed
    ...
    if is_clean:
        ...
        trajectory = clean_trajectory(raw_trajectory)
        trajectory_type = 'cleaned' if len(trajectory) > 1 else 'cluster'
    ...
    return {
        ...
        'primary_park': park_id, 'affected_parks': [park_id],
        'cross_border': False, 
        'first_point': first_point[:2] if isinstance(first_point, list) else first_point,
        'trajectory_type': trajectory_type,  # 'clean', 'cleaned', or 'cluster'
...
def main():
    ...
    log("Fire Trajectory Builder v4 (with zigzag removal)")
    log("Loading parks...")
    parks = load_parks()
    log(f"Loaded {len(parks)} parks")
    log(f"Params: {CLUSTER_KM}km cluster, {BASE_LINK_KM}+{LINK_KM_PER_DAY}km/day link")
    log(f"        {MAX_GAP_DAYS}d max gap, {MAX_DIRECTION_CHANGE}° max turn")
    ...
    for i, park_id in enumerate(park_ids):
        ...
        for g in groups:
            stats[g.get('trajectory_type', 'cluster')] += 1
    ...
    log(f"\nDone! Total: {total_groups} groups")
