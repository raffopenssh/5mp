...
test_query "fire_detections exists" "SELECT COUNT(*) FROM fire_detections LIMIT 1" "nonempty"
test_query "park_fire_analysis exists" "SELECT COUNT(*) FROM park_fire_analysis LIMIT 1" "nonempty"
...
yellow "\n=== Data Integrity ==="
test_query "fire years range" "SELECT MIN(substr(acq_date,1,4))||'-'||MAX(substr(acq_date,1,4)) FROM fire_detections" "nonempty"
...
test_index_used "fire spatial query" "SELECT * FROM fire_detections WHERE latitude BETWEEN -5 AND 5 AND longitude BETWEEN 20 AND 30 LIMIT 10" "idx_fire"
...
test_query "rivers for Chinko" "SELECT COUNT(*) FROM park_rivers WHERE park_id='CAF_Chinko'" "nonempty"
