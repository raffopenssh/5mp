...
curl -s "http://localhost:8000/api/parks/COD_Virunga/fire-narrative?pwd=test2026" | jq '.summary, .hotspots[:2], .trend'

# Deforestation narrative (with trends!)
curl -s "http://localhost:8000/api/parks/COD_Virunga/deforestation-narrative?pwd=test2026" | jq '.trend_direction, .hotspots[:2]'

# Settlement narrative
curl -s "http://localhost:8000/api/parks/COD_Virunga/settlement-narrative?pwd=test2026" | jq '.conflict_risk, .largest_settlements[:3]'

# Park stats (now with deforestation!)
curl -s "http://localhost:8000/api/parks/COD_Virunga/stats?pwd=test2026" | jq '.deforestation'
```
...
- **Testing VM:** https://fivemp-testing.exe.xyz:8000/?pwd=test2026 (larger dataset)

### Local Development
- **App:** http://localhost:8000/?pwd=test2026

### Passwords
- `test2026` - Primary access
- `REDACTED_PWD` - Alternative
- `REDACTED_PWD` - Alternative
...
```bash
curl -L -o db.sqlite3.new "https://fivemp-testing.exe.xyz:8000/static/downloads/5mp_data.sqlite3?pwd=test2026"
...
curl "http://localhost:8000/api/parks/COD_Virunga/fire-narrative?pwd=test2026&year=2020"

# Deforestation with date range
curl "http://localhost:8000/api/parks/COD_Virunga/deforestation-narrative?pwd=test2026&from=2015-01-01&to=2020-12-31"

# Grid effort data (for map)
curl "http://localhost:8000/api/grid?pwd=test2026&year=2024"
```
